...
namespace FirstAttempt {
	class ArcBallCamera : Camera {
		...
		public ArcBallCamera(Vector3 Target, float RotationX, float RotationY, float MinRotationY, float MaxRotationY, float Distance, float MinDistance, float MaxDistance, GraphicsDevice graphicsDevice):base(graphicsDevice){
			...
			//Lock the distance between min and max values
		...
		public override void Update() {
			...
			Matrix rotation = Matrix.CreateFromYawPitchRoll(RotationX, RotationY, 0f);

			//Translate down the Z axis by the desired distance between camera and object, then rotate that vector to find the camera offset from the target
