...
namespace FirstAttempt {
	class ChaseCamera : Camera {
		...
		public override void Update() {
			//Sum the rotation of the model and camera to ensure it is rotated to the correct position relative to the model's rotation
			...
			//Calculate the position the camera wwould be without the spring value, using the rotation matrix and target position
			Vector3 desiredPosition = FollowTargetPosition + Vector3.Transform(PositionOffset, rotation);
			//Interpolate between the currentPosition and the desired position
